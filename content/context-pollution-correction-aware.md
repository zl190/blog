---
title: "The Context Pollution Problem Nobody Talks About"
created: 2026-04-02
date: 2026-04-02
tags: [AI, claude-code, context-windows, context-pollution, microcompact]
topics:
  - AI
qc: passed
spec: "Wrong outputs survive correction in context, competing with the fix — no major framework does correction-aware compression | creator — observed pattern, prototyped fix | developers building conversational agents"
---
...
**Lost in the middle.** Liu et al. (2023, "Lost in the Middle: How Language Models Use Long Contexts") documented that transformer models weight beginning and end context more heavily than middle context. As the conversation grows, the wrong response from turn 2 migrates toward the middle of the window — exactly where it has the most damaging effect.
...
`compact.ts` handles full compaction: when context gets heavy, summarize all old messages into a brief. `autoCompact.ts` fires this automatically as context approaches the window limit — the trigger is buffer-based (context window minus a fixed token reserve), not a percentage threshold. Both are blunt instruments — they compress everything, wrong and right alike.
...
Nobody has built this. ChatGPT has message editing — you can manually fork the conversation at any point, which is the right intuition. But it's user-initiated and it deletes the wrong branch entirely. LangChain has `memory.clear()` and token/window-based pruning, but nothing selective by correctness. MemGPT has memory editing but at the long-term store level, not the in-context message array. No major framework I've found does *automatic* correction-aware compression.
...
def detect_correction(user_message: str) -> bool:
    ...
    patterns = ["no,", "wrong", "I meant", "actually", "not quite"]
    return any(p in user_message.lower() for p in patterns)

def correction_microcompact(messages: list, correction_msg: str) -> list:
    """Replace wrong assistant msg + correction with correction note."""
    for i in range(len(messages) - 1, -1, -1):
        if messages[i]["role"] == "assistant":
            wrong = messages[i]["content"][:200]
            messages[i] = {
                "role": "user",
                "content": (f"[Correction: Previous response wrong. "
                    f"User clarified: {correction_msg}. "
                    f"Wrong (truncated): {wrong}...]")}
            break
    return messages
```
...
*Next issue: what Berkeley's 45 lectures across 3 semesters never cover — and why it matters for anyone building long-running agents. Subscribe to follow the series.*
